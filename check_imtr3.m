...
counter_tr=0;
for ind1=1:N
    hit_trace(ind1)=tracefilter(files(ind1).name,130,[1 1600]);
    pic=monopic(imread((['pic' files(ind1).name(6:end) '.jpg'])));
    hit_image(ind1)=centroid(pic,4);
    if hit_image(ind1)>0
        if hit_trace(ind1)>0
            fprintf(fid_both,'%s \n',[files(ind1).name(6:end) '    ' num2str(hit_image(ind1))]);
