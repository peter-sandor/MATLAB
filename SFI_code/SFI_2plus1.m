...
% *******************************************
function output = SFI_2plus1(input)
...
params.w0C3 = 2*pi*1110 + params.EMAX/params.HBAR/2*1E-12;
params.w0C1 = 2*pi*1020 + params.EMAX/params.HBAR/2*1E-12;
...
%     title(titlestr);
    ...
    plot(times(TI1:TI2),params.n01*params.wField/2/pi*ones(size(times(TI1:TI2))),'r--');
    plot(times(TI1:TI2),params.n0C1*params.wField/2/pi*ones(size(times(TI1:TI2))),'r--');
    xlabel('Time from Pulse Peak [ps]');
    ylabel('Frequency [THz]')
    titlestr= 'Energy levels and multiphoton transition frequencies';
    title(titlestr);
    legend('|1>','|3>','|C1>','|C3>',[num2str(params.n01) '\cdot h\nu_{field}'],[num2str(params.n0C1) '\cdot h\nu_{field}'])
%     plot(t,abs(s_ion).^2);
