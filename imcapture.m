...
function [img, msg] = imgOnly(opt, hAxes, varargin)
    ...
    if (~isequal(DAR, [1 1 1]))
        set(hAxes, 'DataAspectRatio', DAR);
    end     % Reset it if its the case
...
function [all_axes, cbFullWidth, cbAx] = getAllAxes(hFig)
    ...
    if (~isempty(cbAx))
        h_Ylabel = get(cbAx,'Ylabel');
        units_save = get(h_Ylabel,'units');
        set(h_Ylabel,'units','pixels');
        Ylabel_pos = get(h_Ylabel,'Extent');
