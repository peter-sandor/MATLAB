function varargout = calc2D(varargin)
...
% Subtract background, interpolate to have equidistant spacing in frequency space, divide probe channel by ref, take difference of
% phase cycles, then multiply signal back with probe spectra averaged over all pump-pump delays
% procpath='.\';
% required files for processing: data, Np.mat, axes.mat, frame.mat, w1.dat,
% w2.dat. They can be supplied in a third optional argument, as different
% fields of struct AUX.
c=300; % [nm/fs]
if nargin==0
    load([procpath 'data']);
    procpath='.\';
