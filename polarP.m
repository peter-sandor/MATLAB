...
%         end
        ...
        rinc = (rmax - rmin) / rticks;
         for i = 0:1:floor(rmax)% (rmin + rinc) : rinc : rmax
            hhh = line(xunit * i, yunit * i, 'LineStyle', '--', 'Color', 'k', 'LineWidth', 1, ...
                'HandleVisibility', 'off', 'Parent', cax);
            text(double(i), 0, ...
...
%                 'HandleVisibility', 'off', 'Parent', cax);
        for i = 1 : Nspoke
            text(double(rt * (-snt(i))),double(rt * cst(i)), int2str(i * 180/Nspoke),...
                'HorizontalAlignment', 'center', ...
                'HandleVisibility', 'off', 'Parent', cax);
            if i == length(th)
                loc = int2str(0);
            else
                loc = int2str(180 + i * 180/Nspoke);
            end
            text(double(-rt * (-snt(i))),double(-rt * cst(i)), loc, 'HorizontalAlignment', 'center', ...
                'HandleVisibility', 'off', 'Parent', cax);
        end
