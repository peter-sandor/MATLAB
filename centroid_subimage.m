function varargout = centroid_subimage(pic,thrs,Ndivide)
...
for ind1=1:Ndivide(1)
    for ind2=1:Ndivide(2)
        ...
        if ind1<Ndivide(1) && ind1>1
            zone_y=(ind1-1)*B(1)+(1-overlap):(ind1*B(1)+overlap);
        elseif ind1==1 && ind1~=Ndivide(1)
            zone_y=(ind1-1)*B(1)+1:(ind1*B(1)+overlap);
        elseif ind1==Ndivide(1) && ind1~=1
        ...
        if ind2<Ndivide(2) && ind2>1
            zone_x=(ind2-1)*B(2)+(1-overlap):(ind2*B(2)+overlap);
        elseif ind2==1 && ind2~=Ndivide(2)
            zone_x=(ind2-1)*B(2)+1:(ind2*B(2)+overlap);
        elseif ind2==Ndivide(2) && ind2~=1
        ...
        temp2=centroid_peter(pic2,thrs); % this is the core of the code: determine hit coordinates for a sub-image
        if size(temp2,1)==1
            temp4=temp2+offset;
