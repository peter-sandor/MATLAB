...
for ind2 = 1:Nspec
    ...
    Ekin = Edata1{ind2}(:,1);
    E_maxyield0(ind2) = mean(Ekin(Edata_norm{ind2}(1:value2index(Ekin,E_cut)) == max(Edata_norm{ind2}(1:value2index(Ekin,E_cut)))));
...
%         % use a predetermined maximum plateau yield and the tail end fit to
%         % determine the cutoff
%         intersect_plat_fit(ind1) = (log(max_plat_yield/max(Edata1{length(ind_select2)}(:,2))) - FIT_sp(ind1,4).m(1))/FIT_sp(ind1,4).m(2);
%         E_cutoff(ind1) = intersect_plat_fit(ind1) - E0(ind1);
        ...
        % determine the cutoff
        intersect_plat_fit(ind1) = (log(max_plat_yield/max(Edata1{length(ind_select2)}(:,2))) - FIT_sp(ind1,4).m(1))/FIT_sp(ind1,4).m(2);
        E_cutoff2(ind1) = intersect_plat_fit(ind1) - E0(ind1);
    else
        E_plateau(ind1) = 0;
...
set(ax1,'Linewidth',linewidth,'FontSize',fontsize);
legend(['F.E. from slope = ' num2str(roundP(field_enh0,1))  char(177) num2str(roundP(field_enh0_std_chi_sq, 3))])
title('Keldys claculated separately for every intensity');
if flag_save
...
xlabel('keldysh')
ylabel('Field enhancement')
suptitle('Keldys claculated separately for every intensity');
if flag_save
...
set(ax1,'Linewidth',linewidth,'FontSize',fontsize);
% legend(['F.E. from slope = ' num2str(roundP(field_enh0,1))  char(177) num2str(roundP(field_enh0_std_chi_sq, 3))])
legend(['F.E. mean = ' num2str(roundP(mean(field_enh_oldw(4:12)),1)) ' ,F.E. std = ' num2str(roundP(std(field_enh_oldw(4:12)),1)) ' (points:4-12)'])
