...
%varargout is a variable length output argument list. 
tic;

a0=5.29e-11; % [m]
eps0=1/4/pi;
c_au=1/137;
t_scale=24.18884e-18; % [s] 1 atomic unit of time
I_scale=3.51e4; % conversion from TW/cm2 to atomic units (1 a.u.=35.1e15 TW/cm2)
h_SI=6.626e-34; % [J*s]
c_SI=2.99792e8; % [m/s]
kB=1.38e-23; % [J/K]

input=varargin{1}; %Look at run_solve_align_symmtop.m which has values for input laser intensity, fwhm, etc. 
...
sigma1=2*input.laser_fwhm1/(2*sqrt(2*log(2))); %THz pulse duration
sigma2=2*input.laser_fwhm2/(2*sqrt(2*log(2))); % Laser pulse duration
Jvec=0:input.maxJ; %array of J states starting at J=0 to Jmax in steps of 1
Tlimit=2*input.laser_fwhm2; %Time limit vs maxdelay?
Ndelay0=100;
t_ip=-Tlimit:(2*Tlimit)/(Ndelay0-1):Tlimit;  %Delays while interacting with laser pulse, 
t_free=map2colvec(Tlimit+input.timestep:input.timestep:input.maxdelay); % delays afterward
...
% for this part only, use SI units % Check this later, these could be in
% [m]?
...
E_SI0=h_SI*c_SI*100*(B*Jvec.*(Jvec+1)+D*Jvec.^2.*(Jvec+1).^2); %Energy for a linear molecule at every value of J, so this is a vector of energy values.
probs0_JMK=exp(-E_SI0/kB/input.Trot)/sum(exp(-E_SI0/kB/input.Trot)); % 100 converts from cm^-1 to m^-1. 
MaxJ0=max(vec2ind(probs0_JMK>max(probs0_JMK)/1000))+1;% This piece basically
% looks through the probabilities and eliminates ones which are below
% 1/1000 times the max probability. vec2ind() returns the indices of the
% value 1. So as long as probs0_JMK>max(), then it will output a 1, then
% max(probs0_JMK>max()) returns the largest index essentially
% corresponiding to the largest J value, since J's are integers and can be 
% represented by the indices of the probs_0JMK array. 
MaxM=MaxJ0; %Largest value of M can only be largest value of J. 
if flag_linear
    MaxK=0; % For linear molecules, K = 0. 
else
    MaxK=MaxJ0;
end
Mvec=-MaxM:MaxM; % M can range from -M:1:M
Kvec=-MaxK:MaxK; % K can range from -K:1:K
normfactor=0; %Normalization factor 
for indK=1:length(Kvec)
    K=Kvec(indK);
    E_SI(:,indK)=h_SI*c_SI*100*(B*Jvec.*(Jvec+1)+(A-B)*K^2+D*Jvec.^2.*(Jvec+1).^2); %Rotational Kinetic Energy of symmetric top
    normfactor = normfactor + sum(map2colvec(2.*Jvec(Jvec>=abs(Kvec(indK)))+1).*exp(-E_SI(Jvec>=abs(Kvec(indK)),indK)/kB/input.Trot));
    % This is a partition function with the degeneracy. 
    % Confused on size of E_SI. 
end
...
for indK=1:length(Kvec)
    ...
    % This is stores boltzmann population probabilities for all J-values
    % at a given value of K. 
    % Confusion, why not insert the Jvec>=abs(Kvec(indK)) instead of the :
    % on line 73, where E_SI is defined?
    
end
if flag_linear
    probs_JK=probs_JMK.*map2colvec(((0:input.maxJ)*2+1));
    %This is the total probability with degeneracy being in state J. Since
    %energy only depends on J for a linear molecule 
else
    probs_JK=probs_JMK.*extend((0:input.maxJ)*2+1,length(Mvec));
    %  This is the total probability with degeneracy being in state J and
    %  includes K index because energy for a symmtop depends on the K
    % quantum number. 
end
% end of SI part: Boltzmann Probability Distribution done in SI. 
...
cJ1J=2*pi*sqrt(2/3)*cJ1J(:,input.maxJ-MaxM+1:input.maxJ+MaxM+1,input.maxJ-MaxK+1:input.maxJ+MaxK+1);
cJ2J=pi/3*sqrt(2/5)*cJ2J(:,input.maxJ-MaxM+1:input.maxJ+MaxM+1,input.maxJ-MaxK+1:input.maxJ+MaxK+1); %Add a +1/3 to the end of this? Refer to Bob's symm top notes top of p.2
cJ1Jp1=2*pi*sqrt(2/3)*cJ1Jp1(:,input.maxJ-MaxM+1:input.maxJ+MaxM+1,input.maxJ-MaxK+1:input.maxJ+MaxK+1);
cJ2Jp1=pi/3*sqrt(2/5)*cJ2Jp1(:,input.maxJ-MaxM+1:input.maxJ+MaxM+1,input.maxJ-MaxK+1:input.maxJ+MaxK+1);
cJ2Jp2=pi/3*sqrt(2/5)*cJ2Jp2(:,input.maxJ-MaxM+1:input.maxJ+MaxM+1,input.maxJ-MaxK+1:input.maxJ+MaxK+1);
...
function cJmk_dot = Hamiltonian(t,cJmk) % include THz orientation field
    ...
    kin_nrg=E_au(:,indK); %Rotational kinetic energy. 
