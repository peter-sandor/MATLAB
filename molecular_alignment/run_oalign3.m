...
string_towrite{4}=num2str(input_params.peaksep,'%.1f'); % full cycle peak separation [ps]
string_towrite{5}=num2str(input_params.laser_int,'%.1e'); % laser peak intensity in [W/cm^2]
string_towrite{6}=num2str(input_params.laser_fwhm); % laser pulse duration intensity FWHM in [ps]
string_towrite{7}=num2str(input_params.laser_delay,'%.1f'); % delay of laser after HCP in [ps]
string_towrite{8}=num2str(input_params.polar_anisotropy,'%.1f'); % polarizability anisotropy [atomic units]
...
string_towrite{14}=[num2str(input_params.maxdelay,'%.1f') ',' num2str(input_params.delaystep)]; % max timedelay and delay stepsize in [ps]
string_towrite{15}=num2str(input_params.step_acc,'%.1e'); % single step accuracy
