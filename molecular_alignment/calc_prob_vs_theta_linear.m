function varargout = calc_prob_vs_theta_linear(amps,JM_probs,t_ip,t_free,consts,Ylm,theta)
...
for ind5=1:Ndelay
    for ind6=1:size(amp1,4)
        Ylm_sum(ind5,:)=Ylm_sum(ind5,:) + JM_probs(ind6)*map2rowvec(sum(squeeze(abs(squeeze(sum(Ylm_red.*permute(extend(squeeze(amp1(ind5,:,:,ind6)),Ntheta),[3 1 2]),2))).^2),2));
    end
...
for ind5=1:Ndelay2
    E_au_ext=extend(exp(-1i*E_au*(t2(ind5)-t1(end))),size(amp1,3));
    for ind6=1:size(amp1,4)
%         temp1(ind5,:,:,ind6)=abs(squeeze(sum(Ylm_red.*permute(extend(squeeze(amp1(end,:,:,ind6)).*E_au_ext,Ntheta),[3 1 2]),2))).^2;
        Ylm_sum_ff(ind5,:)=Ylm_sum_ff(ind5,:) + JM_probs(ind6)*map2rowvec(sum(squeeze(abs(squeeze(sum(Ylm_red.*permute(extend(squeeze(amp1(end,:,:,ind6)).*E_au_ext,Ntheta),[3 1 2]),2))).^2),2));
    end
